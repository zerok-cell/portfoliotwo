...
const Footer = () => {
  ...
  return (
    <Flex
      p={10}
      wrap="nowrap"
      direction={width < adaptiveTable.sm ? 'column' : 'row'}
      justify="space-between"
      align="center"
      id="Footer"
      className="Footer Footer_noisy"
    >
      <Flex
        className="Footer-Logotype"
        visibleFrom="sm"
        w={width < adaptiveTable.sm ? '100%' : 'fit-content'}
        gap={100}
        justify="start"
        align="center"
        wrap="nowrap"
      ...
      <Flex gap={20} wrap="nowrap" align="center" h="max-content" w="max-content" justify="end">
        {data.map((item, index) => (
          <FooterButtonSocial
            ...
            copiedTextDisplay={item.copiedTextDisplay}
