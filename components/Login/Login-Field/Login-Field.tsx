import {InputValidatorType, LoginProps, LoginPropsZod, LoginPropsZodType} from "@/types/zodTypes/inputValidator";
import {Input} from "@mantine/core";

const LoginField = ({ form, lb, pl, ds, tp, nameField, er }: LoginProps & LoginPropsZodType<InputValidatorType>) => {
    /**
    ...
    return (
        <form.Field  name={nameField}>
            {(field) => (
        <Input.Wrapper error={form.fieldInfo[nameField]?.instance?.getMeta().errors?.join('. ')} w="100%" onBlur={field.handleBlur} label={validateProps.lb} description={validateProps.ds}>
    <Input
